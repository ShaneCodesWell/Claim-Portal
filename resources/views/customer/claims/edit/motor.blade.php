@php
    $f = $claim->form_data; // shorthand — all pre-fill values come from here
@endphp
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="overflow-hidden border border-gray-200">
                    ...
                    <div class="bg-[#0b529d] px-8 py-2.5 flex items-center justify-center gap-4">
                        ...
                        <p class="text-[13px] font-medium tracking-widest uppercase text-white whitespace-nowrap">
                            Motor Accident Report Form — Edit
                        </p>
                        <div class="flex-1 border-t border-white/20"></div>
                    </div>
                    ...
                    <div class="bg-amber-50 border-b border-amber-200 px-8 py-2 text-center">
                        <p class="text-[11.5px] text-amber-700 font-medium">
                            ...
                            You are editing claim <strong>{{ $claim->claim_number }}</strong>.
                            All fields are pre-filled with your original submission.
                        </p>
                    </div>
                </div>
                ...
                <div class="py-6 px-12">
                    ...
                    <form id="motorForm" data-action="{{ route('claims.update', $claim) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <section class="mb-8">
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Is the vehicle the subject
                                    of a hire purchase or loan agreement? <span class="text-red-500">*</span></label>
                                <div class="flex gap-4">
                                    <label class="flex items-center">
                                        <input type="radio" name="hire_purchase" value="yes"
                                            class="conditional-radio mr-2" data-target="financeCompanySection"
                                            {{ ($f['hire_purchase'] ?? '') === 'yes' ? 'checked' : '' }}>
                                        <span>Yes</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="hire_purchase" value="no"
                                            class="conditional-radio mr-2" data-target="financeCompanySection"
                                ...
                                <div id="financeCompanySection"
                                    ...
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Name of finance company
                                        or lending organization <span class="text-red-500">*</span></label>
                                    <input type="text" name="finance_company"
                                        value="{{ $f['finance_company'] ?? '' }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                                </div>
                            </div>

                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-1">State fully the purpose for
                                    which the vehicle was being used. <span class="text-red-500">*</span></label>
                                <textarea name="vehicle_purpose" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">{{ $f['vehicle_purpose'] ?? '' }}</textarea>
                            </div>

                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Was the vehicle being used
                                    with your consent? <span class="text-red-500">*</span></label>
                                <div class="flex gap-4">
                                    <label class="flex items-center">
                        ...
                        <section class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="license_issue_date"
                                        value="{{ $f['license_issue_date'] ?? '' }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                                </div>
                            </div>

                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Who was driving at the time
                                    of accident? <span class="text-red-500">*</span></label>
                            ...
                            {{-- Driver Details Self --}}
                            <div id="driverDetailsSelf"
                                class="{{ ($f['driver_type'] ?? '') === 'self' ? '' : 'hidden' }} mb-6">
                                <h4 class="text-md font-semibold text-gray-800 mb-3">DRIVER DETAILS (from your profile)
                                </h4>
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div><label class="block text-xs font-medium text-gray-600">Address</label>
                                            <input type="text" readonly value="Old Ashongman, Accra Ghana"
                                                class="w-full px-3 py-2 bg-gray-100 border border-gray-200 rounded text-gray-700">
                                        </div>
                            ...
                            <div id="driverInsuranceSection"
                                class="{{ !empty($f['driver_insurance_details']) ? '' : 'hidden' }} mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-1">State name and address of
                                    the insurer of the person driving <span class="text-red-500">*</span></label>
                        ...
                        <section class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <input type="time" name="accident_time"
                                        value="{{ $f['accident_time'] ?? '' }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                                </div>
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Exact location of incident
                                    <span class="text-red-500">*</span></label>
                                <input type="text" name="exact_location" value="{{ $f['exact_location'] ?? '' }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">How many people were in
                                    your vehicle? <span class="text-red-500">*</span></label>
                                <input type="number" name="people_in_vehicle"
                                    value="{{ $f['people_in_vehicle'] ?? '' }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">If you were not in the
                                    vehicle, when was the accident reported to you?</label>
                                <input type="text" name="report_date" value="{{ $f['report_date'] ?? '' }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Give full description of
                                    how the accident happened <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">In your opinion, was the
                                    accident caused by you or your driver? <span class="text-red-500">*</span></label>
                                <input type="text" name="fault_person" value="{{ $f['fault_person'] ?? '' }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Describe the damage to your
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">State exact current
                                    location of the damaged vehicle <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Address <span
                                                class="text-red-500">*</span></label>
                                        <input type="text" name="repairer_address"
                                            value="{{ $f['repairer_address'] ?? '' }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                                    </div>
                                </div>
                            </div>
                        ...
                        <section class="mb-8">
                            ...
                            <div class="mb-8">
                                ...
                                <div id="yourVehicleInjuredPersons" class="space-y-4">
                                    ...
                                    @forelse($injuredPersons as $i => $person)
                                        <div
                                            class="injured-person-row border border-gray-200 rounded-lg p-4 bg-gray-50">
                                        ...
                                        <div
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                                                <div><label class="block text-sm font-medium text-gray-700 mb-1">Full
                                                        Name</label>
                                                    <input type="text"
                                                        name="other_vehicle_injured[{{ $i }}][name]"
                                                        value="{{ $person['name'] ?? '' }}"
                                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                                </div>
                                                <div><label
                                                        class="block text-sm font-medium text-gray-700 mb-1">Age</label>
                                                    <input type="number"
                                                        name="other_vehicle_injured[{{ $i }}][age]"
                                                        value="{{ $person['age'] ?? '' }}"
                                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                                </div>
                                                <div><label
                                            ...
                                            <div><label class="block text-sm font-medium text-gray-700 mb-1">Extent
                                                    of
                                                    Injuries</label>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Full Name <span
                                                class="text-red-500">*</span></label>
                                        <input type="text" name="owner_fullname"
                                            value="{{ $f['owner_fullname'] ?? '' }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                    </div>
                                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Address <span
                                                class="text-red-500">*</span></label>
                                    ...
                                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Telephone <span
                                                class="text-red-500">*</span></label>
                                        <input type="tel" name="owner_telephone"
                                            value="{{ $f['owner_telephone'] ?? '' }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                    </div>
                                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Policy No. <span
                                                class="text-red-500">*</span></label>
                                        <input type="text" name="owner_policy"
                                            value="{{ $f['owner_policy'] ?? '' }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Details of damage to this
                                    vehicle <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex gap-4">
                                    <label class="flex items-center"><input type="radio" name="claim_made"
                                            value="yes" class="conditional-radio mr-2"
                                            data-target="claimMadeSection"
                                ...
                                <div id="claimMadeSection"
                                    ...
                                    <label class="block text-sm font-medium text-gray-700 mb-1">State particulars below
                                        <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Was the accident reported
                                    to the police? <span class="text-red-500">*</span></label>
                                <div class="flex gap-4">
                                    <label class="flex items-center"><input type="radio" name="police_report"
                                            value="yes" class="conditional-radio mr-2"
                                            data-target="policeReportSection"
                                ...
                                <div id="policeReportSection"
                                    ...
                                    <label class="block text-sm font-medium text-gray-700 mb-1">State when and where
                                        reported <span class="text-red-500">*</span></label>
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Name Police Officer who
                                    took particulars <span class="text-red-500">*</span></label>
                                <input type="text" name="officer_details"
                                    value="{{ $f['officer_details'] ?? '' }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            </div>

                            {{-- Conditional: Indemnifying policy --}}
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Do you hold more than one
                                    policy indemnifying you? <span class="text-red-500">*</span></label>
                                <div class="flex gap-4">
                                    <label class="flex items-center"><input type="radio" name="indem_policy"
                                            value="yes" class="conditional-radio mr-2"
                                            data-target="indemPolicySection"
                        ...
                        {{-- SECTION 5: EXISTING DOCUMENTS + NEW UPLOADS --}}
                        <section class="mb-8">
                            <h3 class="text-lg font-semibold text-gray-800 mb-4 border-b border-gray-200 pb-2">
                            ...
                            @if ($claim->documents->isNotEmpty())
                                <div class="mb-4">
                                    <p class="text-sm font-medium text-gray-700 mb-2">Previously uploaded documents:
                                    </p>
                                    <div class="space-y-2">
                                        @foreach ($claim->documents as $doc)
                                            <div
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <i
                                                        class="fas {{ str_contains($doc->mime_type, 'pdf') ? 'fa-file-pdf text-red-400' : 'fa-image text-blue-400' }}"></i>
                                                    <span
                                                        class="text-sm text-gray-700">{{ $doc->original_name }}</span>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <button type="button"
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition cursor-pointer"
                                ...
                                <input type="file" id="imageUpload"
                                    accept="image/jpeg,image/png,image/gif,application/pdf" multiple class="hidden">
                            </div>
                        ...
                        <section class="mb-8">
                            ...
                            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
                                <p class="text-xs text-gray-700 leading-relaxed mb-4">I declare that the above
                                    statement is true in all respects to the best of my knowledge and belief...</p>
                            ...
                            <div class="bg-white border-2 border-blue-200 rounded-lg p-4 mb-6">
                                <label class="flex items-start cursor-pointer">
                                    ...
                                    <span class="ml-3 text-xs text-gray-700">I have read and understood the declaration
                                        above. I confirm that all information provided is true and accurate. <span
                                            class="text-red-500">*</span></span>
                                </label>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Date of Declaration
                                        <span class="text-red-500">*</span></label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Digital Signature <span
                                            class="text-xs text-gray-500">(Type your full name)</span> <span
                                            class="text-red-500">*</span></label>
                        ...
                        <div class="mt-8 pt-4 border-t border-gray-200 flex items-center gap-3">
                            <button type="submit"
                                class="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition flex items-center gap-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.markDocumentForDeletion = function(docId, btn) {
                ...
                // Visual feedback — strike through the file name
                row.closest('.flex.items-center.justify-between').classList.add('opacity-40', 'line-through');
                btn.textContent = 'Undo';
                btn.onclick = () => undoDocumentDeletion(docId, btn, row);
            };

            window.undoDocumentDeletion = function(docId, btn, row) {
                const input = document.getElementById(`delete-doc-${docId}`);
                if (input) {
                    input.value = '';
                    input.disabled = true;
                }
                row.closest('.flex.items-center.justify-between').classList.remove('opacity-40',
                    'line-through');
            ...
            document.getElementById('motorForm')?.addEventListener('submit', async function(e) {
                ...
                // Validate first — before building FormData
                ...
                // Append document IDs marked for deletion
                document.querySelectorAll('[id^="delete-doc-"]:not([disabled])').forEach(input => {
                    formData.append('delete_documents[]', input.value);
                });

                // Read the action URL from the form's data-action attribute
                // This is what routes the edit to PUT /claims/{claim} instead of POST /claims
