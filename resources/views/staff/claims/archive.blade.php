<x-layouts.staff>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">
                All resolved claims · approved, rejected, and closed
            </p>
        </div>
        <div class="flex items-center gap-3">
            <form method="GET" action="{{ route('staff.claims.archive') }}" id="archiveSearch">
                <div class="relative">
    ...
    <div class="flex flex-wrap gap-2 mb-6 border-b border-gray-200 pb-2">
        ...
        'closed' => 'Closed',
            ...
            <a href="{{ route('staff.claims.archive', array_merge(request()->only('search'), ['status' => $value])) }}"
                    ...
                    @forelse($claims as $claim)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-4 text-sm text-gray-700">
                                {{ $claim->committeeDecidedBy?->name ?? ($claim->assignedTo?->name ?? '—') }}
                            </td>
                            ...
                            <td colspan="8" class="px-6 py-10 text-center">
                                <div class="flex flex-col items-center justify-center gap-3 text-gray-500">
                                    <div class="h-14 w-14 flex items-center justify-center rounded-full bg-gray-100">
                                        <i class="fas fa-archive text-xl text-gray-400"></i>
                                    </div>
                                    <div class="text-sm font-medium text-gray-700">No resolved claims found</div>
                                    <div class="text-xs text-gray-400 max-w-xs">
                                        Approved, rejected, and closed claims will appear here.
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    <script>
        document.getElementById('searchInput').addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                document.getElementById('archiveSearch').submit();
            }
        });
    </script>
</x-layouts.staff>
