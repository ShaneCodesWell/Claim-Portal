                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <select name="branch_id"
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ isset($department) && $department->branch_id == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="department_head_id"
                                ...
                                @foreach ($staffMembers as $staff)
                                    <option value="{{ $staff->id }}"
                                        {{ old('department_head_id') == $staff->id ? 'selected' : '' }}>
                                        {{ $staff->name }}
                                    </option>
                                @endforeach
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" rows="3"
                                class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
                                placeholder="Brief description of the department's responsibilities">{{ $department->description ?? '' }}</textarea>
                        </div>
