                                ...
                                <td class="px-4 py-4 text-right relative" x-data="{ open: false }"
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-transition
                                        ...
                                        <form action="{{ route('departments.destroy', $department->id) }}"
                                            method="POST" class="w-full delete-department-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-8">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Manage access staff for claims operations</p>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            (function() {
                ...
                const params = new URLSearchParams(window.location.search);
                if (params.get('agent_search')) {
                    agentSearchInput.focus();
                    const len = agentSearchInput.value.length;
                    agentSearchInput.setSelectionRange(len, len);
                }
                ...
                agentSearchInput.addEventListener('input', () => {
                    ...
                    timer = setTimeout(() => {
                        ...
                        url.searchParams.set('tab', 'tab-agents');
                        if (agentSearchInput.value.trim()) {
                            url.searchParams.set('agent_search', agentSearchInput.value);
                        } else {
                            url.searchParams.delete('agent_search');
                        }
                        url.searchParams.delete('agents_page');
